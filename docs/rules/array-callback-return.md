...
* `"allowImplicit": false` (default) When set to `true`, allows methods that require a return value to implicitly return `undefined` with a `return` statement containing no expression.
