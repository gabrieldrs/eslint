...
ruleTester.run("array-callback-return", rule, {
    valid: [
        // "[\"foo\",\"bar\"].forEach(function(x) { var a=0; })",
        ...
        // { code: "foo.forEach((x) => { var a=0; })", parserOptions: { ecmaVersion: 6 }},
        // { code: "foo.forEach((x) => { if (a === b) { return;} var a=0; })", parserOptions: { ecmaVersion: 6 }},
        // { code: "foo.forEach(x => {if (isSomething(x)) {bar(x);return;} else {throw new Error(\"x isn't something\");}});", parserOptions: { ecmaVersion: 6 }, options: checkForEach },
